...
4. `faucet`: an interface to the Lattice Devnet/Localnet faucet. It makes it easy to fund addresses on the testnet/localnet
...
Generates Store libraries from a `obelisk.config.ts` file. See the [Store Config and `compgen` documentation](store/config) in the Store section for more details.
...
# to deploy to the lattice devnet
